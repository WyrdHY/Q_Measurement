...
classdef Cao_3
    ...
    properties
       ...
       %Updated on 2026/01/14
        % Also return if using split mode, split g
        % and g/kappa, kappa is  1/total_photon_life time
        is_split_used
        g_MHz_list
    ...
    methods
        function Q_obj=Cao_3(trace_Q,trace_MZI,MZI_FSR,lambda,threshold,corr_type,cutoff_freq) % constructor
            %Updated on 2026/01/14
            % Also return if using split mode, split g
            % and g/kappa, kappa is  1/total_photon_life time
...
%             close all;
            ...
            % Relative frequency (horizontal axis) estimate
            MZI_zeros=find(diff(trace_MZI>MZI_baseline));
            % set 1/4 period dead time
            MZI_zero_current=MZI_zeros(1);
            for jj=2:length(MZI_zeros)
                if MZI_zeros(jj)<=MZI_zero_current+.25*MZI_period
                    MZI_zeros(jj)=0;
                else
                    MZI_zero_current=MZI_zeros(jj);
                end
            end
            MZI_zeros=MZI_zeros(find(MZI_zeros));
            frequency_base=interp1(MZI_zeros,(1:length(MZI_zeros)).',(1:length(trace_MZI)).','linear','extrap');
            frequency_base=(frequency_base-frequency_base(1))/2*MZI_FSR;
            ...
            while 1 % loop over all peaks
                ...
                if ~flag_osc_corr % no 'osc' correction
                    ...
                    if ~flag_split_corr
                        ...
                        peakend=min(peakpos+5*linewidth,trace_length);
                        peakstart=max(peakpos-5*linewidth,1);
                        trace_Q_trunc(peakstart:peakend)=1;
                        trace_Q_tofit=trace_Q(peakstart:peakend);
                        peakrange=find(trace_Q_tofit<trace_Q_baseline(peakstart:peakend)*(1+transmission)/2);
                        g2estimate=(max(peakrange)-min(peakrange))^2/4;
                        peakdirection=sign(max(peakrange)+min(peakrange));
               ...
               %Updated on 2026/01/14
                % Also return if using split mode, split g2
                try             
                    
                        g = sqrt(Q_fit.g2);%Unit: # of MZI frequency
                        g_MHz = g / MZI_fit_T * MZI_FSR;
                        scatter_strength = g/kappa; % Dimentionless hz/hz
                        
                        % Save to list
                        g_MHz_list=[g_MHz_list;g_MHz];
                        scatter_strength_list=[scatter_strength_list;scatter_strength];
                        is_split_used = 1;
                catch 
                    a =1;
                end    
            ...
            Q_obj.frequency_base=frequency_base;
            ...
            %Updated on 2026/01/14
            % Also return gMhz, and g/kappa total
            try
                    Q_obj.g_MHz_list=g_MHz_list(indexorder);
                    Q_obj.scatter_strength_list=scatter_strength_list(indexorder);
                    Q_obj.is_split_used = is_split_used;
            catch 
                a =1;
            end    
        ...
        function plot_Q_stat(Q_obj, target_axes)
            % target_axes is the handle to the UIAxes object where the plot will be displayed
            hold(target_axes, 'on');
            xlabel(target_axes, 'Q (Intrinsic) (M)');
            ylabel(target_axes, 'Transmission');
            grid(target_axes, 'on');
            title(target_axes, 'Q statistics', 'FontSize', 10, 'FontWeight', 'normal');
            ylim(target_axes, [-0.15, 1.05]);
            if ~isempty(Q_obj.modeQ0)
                scatter(target_axes, Q_obj.modeQ0, Q_obj.modeTransmission, 'filled');
            end
            hold(target_axes, 'off');
        end
        
        function plot_trace_stat(Q_obj, target_axes)
            % Hold on to target axes
            hold(target_axes, 'on');
            
            % Set labels and title
            xlabel(target_axes, 'Frequency Estimate (MHz)');
            ylabel(target_axes, 'Trace voltage (V)');
            
            % Set Y limits based on trace_Q data
            ylim(target_axes, max(Q_obj.trace_Q)*[-0.1 1.1]);
            title(target_axes, 'Trace statistics', 'FontSize', 10, 'FontWeight', 'normal');
            
            % Plot if modeQ0 is not empty
            if ~isempty(Q_obj.modeQ0)
                plot(target_axes, Q_obj.frequency_base, Q_obj.trace_Q, 'DisplayName', 'Trace Data');
                
                % Adding text annotations for mode positions
                text_Q = text(target_axes, ...
                              [Q_obj.frequency_base(Q_obj.modePos); Q_obj.frequency_base(Q_obj.modePos)], ...
                ...
                % Updated on 2026/01/14
                % Also return g_MHz, and g/kappa_total
                
                try
                    if Q_obj.is_split_used ~= 1
                        disp('aband')
                        return;
                    end
                
                    idx = Q_obj.modePos(:);
                    xg  = Q_obj.frequency_base(idx);
                    yg  = Q_obj.modeBaseline(:) .* (Q_obj.modeTransmission(:) - 0.10);
                
                    g = Q_obj.g_MHz_list(:);
                    s = Q_obj.scatter_strength_list(:);
                
                    str = arrayfun(@(gg,ss) sprintf('g = %.4g MHz\nsc = %.4g', gg, ss), ...
                                   g, s, 'UniformOutput', false);
                
                    text(target_axes, xg, yg, str, ...
                         'HorizontalAlignment', 'center');
                
                catch ME
                    warning('Optional g/scatter annotation skipped: %s', ME.message);
                end


                      
            % Release the hold on target axes
            hold(target_axes, 'off');
        end

        
        function figure_handle=plot_Q_max(Q_obj, target_axes)
            try
            figure_handle=plot_Q(Q_obj,1,target_axes);
            catch
                plot_Q(Q_obj,1,target_axes);
            end
        end
        
        function plot_Q(Q_obj, n, target_axes)
            % Use the provided UIAxes for plotting
            hold(target_axes, 'on');
            xlabel(target_axes, 'Frequency (MHz)');
            ylabel(target_axes, 'Normalized outputs');
            grid(target_axes, 'on');
            
            if n <= length(Q_obj.modeQ0)
                % Your plotting logic remains the same, just direct all plotting functions to use target_axes
                ...
                xlim(target_axes, [min(plot_base) max(plot_base)] / MZI_fit_T * Q_obj.MZI_FSR);
                scatter(target_axes, plot_base / MZI_fit_T * Q_obj.MZI_FSR, (Q_obj.trace_MZI(peakstart:peakend) - MZI_fit_A0) / MZI_fit_A * 0.2, 'ko');
                plot(target_axes, plot_base / MZI_fit_T * Q_obj.MZI_FSR, (MZI_fit - MZI_fit_A0) / MZI_fit_A * 0.2, 'c', 'LineWidth', 2);
                scatter(target_axes, plot_base / MZI_fit_T * Q_obj.MZI_FSR, Q_obj.trace_Q(peakstart:peakend) / Q_fit_A, 'bo');
                plot(target_axes, plot_base / MZI_fit_T * Q_obj.MZI_FSR, Q_fit / Q_fit_A, 'r', 'LineWidth', 2);
                legend(target_axes, 'MZI interferogram', 'Sinusoidal fit', 'Transmission', 'Lorentzian fit', 'Location', 'best');
                
                Q0 = Q_obj.modeQ0(n);
                Q1 = Q_obj.modeQ1(n);
                QL = Q_obj.modeQL(n);
                transmission = Q_obj.modeTransmission(n);
                title(target_axes, ['Q0=', num2str(Q0, '%.4g'), 'M, Q1=', num2str(Q1, '%.4g'), 'M, Q=', num2str(QL, '%.4g'), 'M, Transmission=', num2str(transmission, '%.4g')], 'FontSize', 10, 'FontWeight', 'normal');
            end
